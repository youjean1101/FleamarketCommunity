...
//페이징 처리
 function callPageSelector(flex, number, paginationleft, paginationright){
        ...
        const numbers = document.querySelector(number); //클릭할 페이지 버튼번호가 생성될 ol
        //변수 추가 이전다음버튼
        const prevPageBtn= document.querySelector(paginationleft);
        const nextPageBtn= document.querySelector(paginationright);
